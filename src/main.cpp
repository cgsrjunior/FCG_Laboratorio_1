...
GLuint BuildTriangles2(); // Constr�i tri�ngulos para renderiza��o
...
int main()
{
    ...
    GLFWwindow* window;
    window = glfwCreateWindow(500, 500, "INF01047 - 00270139 - Cleiber Gustavo Soares Rodrigues Junior", NULL, NULL);
    if (!window)
    {
        glfwTerminate();
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        //Aqui tentamos montar a representacao de um triangulo
        pos=-0.8;
        GLuint vertex_array_object_id = BuildTriangles();
        pos=-0.4;
        GLuint vertex_array_object_id2 = BuildTriangles();
        pos=0;
        GLuint vertex_array_object_id3 = BuildTriangles();
        pos=0.4;
        GLuint vertex_array_object_id4 = BuildTriangles();

        pos=-0.8;
        GLuint vertex_array_object_id0 = BuildTriangles2();
        pos=-0.4;
        GLuint vertex_array_object_id20 = BuildTriangles2();
        pos=0;
        GLuint vertex_array_object_id30 = BuildTriangles2();
        pos=0.4;
        GLuint vertex_array_object_id40 = BuildTriangles2();
        ...
        int clock=seconds%16;

        switch(clock)
        {
            //0000
        case 0:
            glBindVertexArray(vertex_array_object_id0);
            glDrawElements(GL_TRIANGLE_STRIP, vertices+2, GL_UNSIGNED_BYTE, 0);
            glBindVertexArray(vertex_array_object_id20);
            glDrawElements(GL_TRIANGLE_STRIP, vertices+2, GL_UNSIGNED_BYTE, 0);
            glBindVertexArray(vertex_array_object_id30);
            glDrawElements(GL_TRIANGLE_STRIP, vertices+2, GL_UNSIGNED_BYTE, 0);
            glBindVertexArray(vertex_array_object_id40);
            glDrawElements(GL_TRIANGLE_STRIP, vertices+2, GL_UNSIGNED_BYTE, 0);
            break;
        //0001
        case 1:
...
GLuint BuildTriangles()
{
    ...
    //GLfloat PI2 = PI * 2.0f;
    GLfloat NDC_coefficients[28];

    NDC_coefficients[0]=(0.0f+pos);
    NDC_coefficients[1]=-0.2f;
    NDC_coefficients[2]=0.0f;
    NDC_coefficients[3]=1.0f;

    NDC_coefficients[4]=(0.1f+pos);
    NDC_coefficients[5]=-0.2f;
    NDC_coefficients[6]=0.0f;
    NDC_coefficients[7]=1.0f;

    NDC_coefficients[8]=(0.0f+pos);
    NDC_coefficients[9]=0.2f;
    NDC_coefficients[10]=0.0f;
    NDC_coefficients[11]=1.0f;

    NDC_coefficients[12]=(0.1f+pos);
    NDC_coefficients[13]=-0.2f;
    NDC_coefficients[14]= 0.0f;
    NDC_coefficients[15]=1.0f;

    NDC_coefficients[16]=(0.1f+pos);
    NDC_coefficients[17]=0.2f;
    NDC_coefficients[18]=0.0f;
    NDC_coefficients[19]=1.0f;

    NDC_coefficients[20]=(0.0f+pos);
    NDC_coefficients[21]=0.2f;
    NDC_coefficients[22]=0.0f;
    NDC_coefficients[23]=1.0f;

    NDC_coefficients[24]=(-0.05f+pos);
    NDC_coefficients[25]=0.1f;
    NDC_coefficients[26]=0.0f;
    NDC_coefficients[27]=1.0f;
    ...
    // Conversaremos sobre sistemas de cores nas aulas de Modelos de Iluminação.
    GLfloat color_coefficients[vertices*4];
    //config um
    ...
    //
    GLubyte indices[] = {0,1,2,3,4,5,6}; // GLubyte: valores entre 0 e 255 (8 bits sem sinal).
...
GLuint BuildTriangles2()
{
    ...
    GLfloat NDC_coefficients[vertices*4];


    //zero
    for ( int i = 0; i < vertices*4; i+=8)
    {
        NDC_coefficients[i] = (0.05 * cos((i-4)/4*(PI/(vertices/2))))+pos;
        NDC_coefficients[i+1] = (0.15 * sin(((i-4)/4)*(PI/(vertices/2))));
        NDC_coefficients[i+2] = 0.0f;
        NDC_coefficients[i+3] = 1.0f;

        NDC_coefficients[i+4] = (0.15 * cos((i-4)/4*(PI/(vertices/2))))+pos;
        NDC_coefficients[i+5] = (0.2 * sin(((i-4)/4)*(PI/(vertices/2))));
    ...
    // Agora repetimos todos os passos acima para atribuir um novo atributo a
    // cada v�rtice: uma cor (veja slide 115 do documento "Aula_04_Modelagem_Geometrica_3D.pdf").
